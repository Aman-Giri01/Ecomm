...
//   existingCart.items = [];
//   await existingCart.save({ validateBeforeSave: false });
//   // this validateBeforeSave will not check for the validation against schema. it will save the document.
...
//     // existingCart.items.length === 0 ? "No Product in Cart" : existingCart
//      flatArray.length === 0 ? "No Product in Cart" : existingCart
//   ).send(res);
// });
...
export const addToCart = expressAsyncHandler(async (req, res, next) => {
  ...
  new ApiResponse(201, "Product Added Successfully", existingCart).send(res);
});
...
export const getCart = expressAsyncHandler(async (req, res, next) => {
  ...
  //  Removed -_id so product._id is included in the populated result
  let existingCart = await CartModel.findOne({ userId }).populate({
    path: "items.productId",
