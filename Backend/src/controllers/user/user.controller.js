...
export const registerUser = expressAsyncHandler(async (req, res, next) => {
  ...
  if (isExisting) {
    next(new CustomError(400, "user already exists"));
  }
  ...
  sendEmail(
    ...
    "Email Verification",
    "sample text",
    `<h1>This is for verification</h1><button> <a href="${verification_url}">Click here</a></button>`
  );
...
export const verifyEmail=expressAsyncHandler(async(req,res,next)=>{
  ...
  let hashedEmailToken=crypto.createHash(sha256).update(emailToken).digest("hex");
